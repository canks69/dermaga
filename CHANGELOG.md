...
- **The menu bar item opens a panel, not a menu.** A menu can be styled in exactly two places, the
  text and the icon. Everything else — background, highlight, row height, corners — belongs to macOS,
  so a row carrying a start button is a row that *almost* matches the ones above it, and a menu is
  where people are least forgiving about almost. What hangs under the icon now is a page of the app in
  a frameless window: what is running and what is not, filtered by the project in force and following
  the same rules the window's own lists follow, with start and stop on every row — always visible
  rather than appearing under the pointer, because a panel opened to start something that shows
  nothing to press until the mouse finds the right line is a list, not a panel. The project switcher
  is there, so is starting the services when they are down, and so is an update already downloaded.
  Either mouse button opens it; Escape or a click anywhere else takes it down. It is sized to whatever
  the page turns out to be, and the list scrolls once it would grow past the screen rather than
  running off the bottom of it. Opening it brings the app forward, which is the cost of it being an
  ordinary window: only the active app has a key window, and a panel that is not key cannot be
  dismissed with Escape and answers its first click by taking focus instead of doing what was pressed.

### Fixed

- **The menu bar and the sidebar could disagree about which project was in force.** Preferences are
  announced now — the agent sends `settings.changed` on every save and the window applies what it did
  not write itself — so switching project in one place switches it in both. Applied only where it
  differs, and never written back: a value written back comes round again as another announcement, and
  with anything correcting what arrives, the correction and the echo take turns forever. That is not
  hypothetical. It ran at several hundred writes a second until the app was killed, and there is a
  test for it now.
- **"All projects" did not survive a restart.** `activeProject` was left out of the settings file when
  it was empty, and empty is exactly what "all" is — the one choice the file could not hold.
...
- **Projects: a point of view over what is on this Mac.** Many repositories, one flat list. A project
  groups what belongs to one piece of work, and everything that already exists is in `default`, which
  is the absence of a project rather than a place things were moved to — nothing was written and
  nothing was touched. A project *filters*: it decides what is shown and what a new thing is named,
  never what anything can reach, and two containers in different projects talk to each other exactly
  as they did before. What is born in one carries its name — `linxpay_whoami`, `linxpay_app:dev`,
  `linxpay_data` — with the underscore Compose uses, because people arrive already reading `myapp_db`
  as "db, in myapp". That prefix costs something, and it is shown in the form for exactly that
  reason: a container's name is its address on every network it joins, so a prefixed container
  answers at `linxpay_db` and not at `db`. The alternative was a second project being unable to hold
  a `dashboard` at all. Each project gets a network, made with it and removed with it while nothing
  is attached — the one part that does something rather than filtering, and what lets services born
  together find each other by name. The rules differ by kind and each says what it is: containers and
  volumes are filed by record, so something that predates projects can join one without being
  touched; an image belongs where it was *built*, and anything pulled stays global, because a project
  that cannot see the image it is about to run is a project getting in the way. There is no moving
  between projects, and that is deliberate — nothing here can be renamed, so a real move meant
  destroying and rebuilding, which is where every half-finished state came from. Marked beta.

### Fixed

- **A release cannot go out without its contributors.** v1.13.0 shipped with no names on its What's
  new entry and nothing reported a problem: the step that works them out could not reach GitHub, and
  "there is nobody to name" and "I could not find out" were the same empty answer. They are not. Every
  release has an author, so an empty answer is a question that did not get through — usually commits
  that are still only on the machine cutting the release. It now stops before the tag when GitHub
  cannot be asked, when nobody could be named, when only *some* authors could be, and when the version
  has no entry to put names into. And the built app is checked at the end, where the question was
  always really being asked: a bundle carrying no credit, or one that does not mention the version
  being released, fails the build. v1.13.0's own line is filled in.
- **A bind mount comes back from the runtime as a bind mount.** The edit form showed the type as
  `volume` and the bound path in the box where a volume name goes. The runtime takes `type=bind` and
  reports it back as `virtiofs`, which is the transport rather than the kind, and the selector fell
  back to its first option on a word it had never heard of. Cosmetic only until Save: what a selector
  shows is what a save sends, so recreating would have asked for a volume named after somebody's
  source directory, and the container would have come back with an empty folder where the work was.
- **An edit that cannot work leaves the container alone.** Editing recreates, and a recreate has to
  delete before it can run — a container's name is its id here, and there is no rename to move the old
  one aside with. Anything the new settings got wrong used to be discovered inside that gap, with the
  container already gone. The container is now built once under a name of its own and thrown away
  before anything is taken apart, which catches an image that has been deleted since and a bind mount
  whose directory has moved. It narrows the gap rather than closing it: layers are unpacked at start,
  so an image whose content is incomplete still fails afterwards.
- **Saving a container stops asking whether to discard it.** Pressing recreate answered with "Leave
  without saving? … Nothing has been created yet." — about a recreate that had just been confirmed and
  started. Throwing an unfinished edit away had the same problem: that *is* the answer to the question
  about leaving, not a reason to be asked it again on the way out.
- **`make dev` no longer reaches for the network to start.** It died on a package that existed and was
  empty, left behind by a download interrupted halfway into a temporary cache. The tools it runs are
  installed with everything else now, so their versions are in the lockfile and starting the app never
  waits on a download.
...
- **The long forms are pages now, not panels laid over what they are about.** Making a container asks
  for thirty things — a name, an image, limits, networks, ports, mounts, a whole environment — and it
  asked for them in a box that had to scroll inside itself while covering the list the container was
  about to appear in. Creating and editing a container, the template catalogue, building an image,
  publishing a hostname and making a machine are each a page of their own now, with the window's
  height to use and the same back link every other page is left by. Where that link goes is wherever
  you opened the form from: run an image from its own page, and leaving puts you back on that page
  rather than on a list of containers.
- **Work that runs for minutes ends on what it is printing.** Pressing Create, Build or Create machine
  used to put you back on a list with a bar in the corner of the title bar, and the only way to read
  what a command was actually saying was a dialog half the window high, opened once it had already
  finished. There is a page for it now — the whole window, following the tail while the run is live
  and leaving off the moment you scroll up to read something. Every row in the task strip opens it,
  running or finished: a build in progress, which is the one people actually watch, had no way in at
  all.
- **Create, Build and Recreate say what they are about to do, and wait.** None of them is destructive
  except Recreate, so this is not the confirmation a delete gets — it is the last cheap moment to read
  the settings back as a sentence rather than as thirty controls, which is where a port typed into the
  wrong box, or a gigabyte that was meant to be a megabyte, is actually noticed. Only what you set is
  read back. Build arguments are named but never quoted: what people put in them is versions and
  tokens, and a dialog is not the place to put a token on screen.
- **Leaving a form you have typed into asks first.** Escape was deliberately left out of these forms so
  that a key reached for by mistake could not throw away ten minutes of typing. That was right and it
  stopped one step short: a form is a page now, which puts the sidebar beside it the whole time it is
  being filled in, and one stray click there did exactly what Escape was stopped from doing. Every way
  of leaving asks now — the sidebar, the search results, a notification opening something else — and
  a form you have just submitted is never asked about, because there is nothing left in it to lose.
- **The sidebar and the title bar follow the theme.** They were near-black whatever the theme was, on
  the grounds that a frame should not change colour with the picture — which in a light window meant a
  black border down two sides of it. The frame is a shade under the page in the light now, and the
  near-black it always was in the dark. Log output keeps its dark box in both, because the colours a
  program prints are pulled towards that ground and half of them wash out on white.
- **A release says who worked on it.** What changed was written down; who by was not, and the names
  were only ever in the git history, which is not where anybody reads a release from. Every version
  from this one on carries a line naming everybody who worked on it, here and on the release page, and
  each name opens that person's GitHub account.

### Fixed

- **The window could lose its agent the moment it opened, and blame the agent for it.** Everything on
  the first screen would fail with "the Dermaga agent stopped" while the agent was still running and
  still answering. What it asked for on opening was every stored scan result, findings and all — two
  busy images here came to 17 MB in one answer, and the window will not read a message that big. It
  asks for the counts now, which is all any list shows; the findings are read on an image's own page,
  which asks for that one result by name.
- **A build that finished while the window was closed had nothing to open.** Which is the one case the
  notification exists for: the build runs for minutes, the window is closed for all of them, and
  clicking "finished" opened a page that said nothing was running under that name. What a command
  printed lived in the window's memory and was written down by the window, when it finished, if it was
  still open. The agent keeps it now, whether or not anybody is watching.
- **The suggestions under the image and volume fields never appeared.** They were the browser's own
  list, and the engine this window is drawn in barely implements it: nothing dropped down, nothing said
  the field had anything to offer, and the size or address written beside each name was thrown away
  unread. The list is drawn by the app now — it narrows as you type, moves with the arrow keys, and
  shows what each one costs to keep or where it answers. What you type is still the value; nothing is
  chosen for you.
- **A dropped folder with a capital letter in its name suggested a tag no registry would accept.**
  Dropping a Dockerfile fills the tag in from the folder it came from, and folders are named by people
  — `Portfolio`, `Client Work`. Registries refuse an uppercase name outright, so the form opened
  already objecting to a field nobody had touched. The suggestion is lowercased and tidied now; what
  you type yourself is still left alone.
