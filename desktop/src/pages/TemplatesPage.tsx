...
import { PLACEHOLDER_WIDTHS, SkeletonBar } from '../components/Skeleton';
 ...
 *
 * A page rather than a dialog over the container list. It is a catalogue --
 * something read through and compared across, which is what a grid of cards on
 * a whole window is for and what a panel with a scrollbar down its middle is
 * not -- and picking from it goes somewhere, so it is a step on the way rather
 * than a question asked in front of where you were.
 *
 * It is reachable without the mouse: type to narrow, arrows to move, Enter to
 * take one.
 */
...
export function TemplatesPage({ route }: { route: Extract<Route, { name: 'templates' }> }) {
  const back = useUIStore((s) => s.back);
  // Picking one leaves for the form it fills in, which is where anything is
  // actually made. Nothing is created from this page.
  ...
  // through what is left, so neither ever asks for the mouse.
  //
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <PageHeader
        ...
        filters={
          <>
            {/* A search box on a page, which no other page here has: the field
                in the title bar asks the resource stores, and the catalogue is
                not one of them -- it is a list of things this Mac does not have
                yet. It is also what the whole keyboard flow hangs off, so it
                takes the caret on arrival. */}
            <label className="relative block min-w-0 flex-1">
              ...
              <input
                ...
                className="input w-full max-w-96 pl-9"
              />
            </label>

            {shown.length > 0 && (
              <span className="flex shrink-0 items-center gap-3 text-tiny text-ink-500">
                <span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-7 py-5">
        {templates === null ? (
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(210px,1fr))] gap-2.5">
            {Array.from({ length: 8 }, (_, at) => (
              <div
                key={at}
                aria-hidden
                className="flex items-start gap-2.5 rounded-xl border border-ink-200 p-3.5 dark:border-ink-800"
              >
          ...
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <div>
              ...
              <p className="mx-auto mt-1 max-w-sm wrap-break-word text-xs leading-relaxed text-ink-600 dark:text-ink-400">
                They are fetched from a catalogue and kept for when you are offline. This Mac has
                not reached it yet — everything else about creating a container works meanwhile.
              </p>
            </div>
            <Button icon={RefreshCw} busy={refreshing} busyLabel="Fetching…" onClick={refresh}>
              Try now
          ...
          <p className="py-10 text-center text-xs text-ink-600 dark:text-ink-400">
          ...
          >
            {shown.map((template, position) => (
              <button
                ...
                onClick={() => newContainer(template.spec)}
...
function cameFrom(from: Route | undefined): string {
  return from?.name === 'images' ? 'Images' : 'Containers';
}
